...
class FillAnswerController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update', 'pregledaj', 'savepoints', 'proveri', 'saveanswer'),
				'users'=>array('@'),
			),
	...
	{
		$model=new FillAnswer;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['FillAnswer']))
		{
			$model->attributes=$_POST['FillAnswer'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('FillAnswer');
	...
	{
		$model=new FillAnswer('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['FillAnswer']))
			$model->attributes=$_GET['FillAnswer'];
	...
	{
		$model=FillAnswer::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='fill-answer-form')
		{
        ...
        public function actionSaveanswer()
        {
            ...
            if(isset($_POST))
            {
                ...
                $model->fill_id = $_POST['fill_id'];
                $model->student_id = Yii::app()->user->id;
		
                if($model->save())
                {
                    header("location: ".$_SERVER['HTTP_REFERER']);
//                    $this->redirect('index.php?r=test/polagaj&test_id=1&type=fill');
                }
            }
            header("location: ".$_SERVER['HTTP_REFERER']);
//            $this->redirect('index.php?r=test/polagaj&test_id=1&type=fill');
...
//                $odgovori = $command->queryAll();
                ...
                $criteria1->select = "t.*";
                $criteria1->join = "JOIN `fill_question` ON (t.fill_id = fill_question.id)";
                $criteria1->addCondition("fill_question.test_id = :test_id");                
                $criteria1->params[':test_id'] = $test_id;
                $criteria1->addCondition("t.student_id = :student_id");
                $criteria1->params[':student_id'] = $student_id;
                
                $odgovori = FillAnswer::model()->findAll($criteria1);
        ...
        {
                $model1 = new FillAnswer;
                $model2 = new FillReply;
                
                $fillanswer_id = $_POST['fillanswer_id'];
                $points = $_POST['FillAnswer']['points'];
                
                if($_POST['comment'] != '')
                {
                    $comment = $_POST['comment'];
                    $model2->user_id = Yii::app()->user->id;
                    $model2->fillanswer_id = $fillanswer_id;
                ...
                $model1 = FillAnswer::model()->findByPk($fillanswer_id);
